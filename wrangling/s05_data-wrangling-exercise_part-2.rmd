...
mutate(sepal_width = sepal_width / 2.54) %>% 
  mutate(petal_width = petal_width / 2.54) %>% 
  mutate(petal_length = petal_length / 2.54) %>% 
  print()
...
this is also rounding!
```{r}
gapminder %>% 
  group_by(continent, year) %>% 
  summarise(min_life = min(round(lifeExp, -1)))
```
